...
pub trait Fixture: Sized {
    ...
    /// evaluated and only the fixtures previously set up are tore down.
    fn set_up() -> crate::Result<Self>;

    /// Clean up method for the fixture.
    ///
    /// This method is called by the end harness after the test logic.
    /// If the `Fixture` has been set up, the test harness will call this
    /// method, even if the test logic failed or paniced.    fn tear_down(self)
    /// -> crate::Result<()> {
...
/// Generic new type wrapper to implement `ConsumableFixture` on any type
...
/// Interface for structure to be setup only once before all tests.
...
impl<F: StaticFixture + 'static> Fixture for &'static F {
    ...
    // Note that this is `&F` being tear down, not `F`.
    fn tear_down(self) -> crate::Result<()> {
        Ok(())
    }
}
