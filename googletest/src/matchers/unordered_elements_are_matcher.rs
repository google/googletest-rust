...
mod tests {
    ...
    // This test verifies that unordered_elements_are can be used as the inner matcher of another
    // matcher, where the inner matcher is given a reference to a local variable. If
    // unordered_elements_are stores its matchers in a Vec with the same type and lifetimes then
    // this fails to compile.
    #[test]
    fn works_with_inner_matcher_that_modifies_the_input() -> googletest::Result<()> {
        // A custom matcher that applies an operation to the input hashmap (filtering out any keys
        // present in `to_filter`) and then applies the inner matcher to the modified input.
