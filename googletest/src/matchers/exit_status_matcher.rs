...
/// Matches an [`ExitStatus`] which corresponds to a process that exited
/// normally with the given exit code.
///
/// ```
...
/// A matcher for `ExitStatus` checking if it corresponds to normal exit with a
/// code.
#[derive(MatcherBase)]
