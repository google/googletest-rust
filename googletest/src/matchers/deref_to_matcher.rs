...
/// Dereference the `actual` value and verifies that the returned reference
/// matches the `inner` matcher.
///
/// ```
/// # use googletest::{matchers::{deref_to, eq}, verify_that};
/// verify_that!(Box::new(123), deref_to(eq(&123)))
/// #    .unwrap()
/// ```
///
pub fn deref_to<Inner>(inner: Inner) -> DerefTo<Inner> {
    DerefTo { inner }
}

/// A matcher which deref a value and verifies that the result match the `inner` matcher.
///
/// See [`deref_to`].
#[derive(MatcherExt)]
pub struct DerefTo<InnerT> {
    pub(crate) inner: InnerT,
}

impl<'a, ActualT, ExpectedT, Inner> Matcher<&'a ActualT> for DerefTo<Inner>
where
