...
/// to any value.
const DISABLE_ABBREVIATION_ENV_VAR: &str = "GTEST_RUST_DISABLE_ABBREVIATION";
...
mod tests {
    ...
    fn abbreviate_does_not_affect_string_if_disabled_by_environment_variable() -> Result<()> {
        std::env::set_var("GTEST_RUST_DISABLE_ABBREVIATION", "yes");
        let input = format!("{}\\n", ".".repeat(ABBREVIATION_LENGTH_THRESHOLD - 1));

        let result = abbreviate(&input);

        let test_result = verify_that!(result, eq(input.as_str()));

        // Teardown: Reset environment so that this test's changes doesn't affect other
        // tests.
        std::env::remove_var("GTEST_RUST_DISABLE_ABBREVIATION");
        test_result
    }
}
