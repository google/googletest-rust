...
use googletest::{field, google_test, verify_that, Result};
...
fn struct_in_other_module_matches() -> Result<()> {
    verify_that!(
        sub::SubStruct { field: 32 },
        field!(sub::SubStruct.field, eq(32))
    )
}
...
fn shows_correct_failure_message_for_wrong_enum_value() -> Result<()> {
    ...
    verify_that!(
        result,
        err(displays_as(contains_substring("which has no field `a`")))
    )
}
