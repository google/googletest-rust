...
impl AccumulatePartsState {
    fn new() -> Self {
        Self { var_num: 0, var_defs: vec![], formats: vec![] }
    }
    ...
    fn accumulate_parts(&mut self, expr: Expr) -> Expr {
        ...
        self.formats.push(quote! {
            ::googletest::assertions::internal::__googletest_format_if_debug_macro!(
                #expr_string,
                #new_expr,
            )
        });
...
pub fn __googletest_macro_verify_pred(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let parsed_string = quote!(#parsed).to_string();

    let mut state = AccumulatePartsState::new();
    let pred_value = state.accumulate_parts(parsed);
    let AccumulatePartsState { var_defs, mut formats, .. } = state;
    ...
    quote! {
        {
            ...
            if (#pred_value) {
                ...
                ::googletest::assertions::internal::report_failed_predicate(
                    #parsed_string,
                    [#(#formats),*]
                        .into_iter()
